...
## 10. Aggressive Pipeline Strategy (v2)

### Type A / Type B Setup Classification

| Setup | Type | Eval Risk | Rationale |
|:------|:-----|:---------|:----------|
| **B-Day IBL Fade** | **A** | **$2,500** | 100% WR (4/4), CVD divergence 100% match, tight stops |
| **Trend Bull VWAP (high confidence)** | **A** | **$2,500** | 3+ acceptance bars, strong trend structure |
| Trend Bull VWAP (standard) | B | $1,500 | 75% WR, wider stops |
| P-Day VWAP Long | B | $1,500 | 75% WR, similar to standard trend |

### Eval Sizing Scheme Comparison (Realistic 70% WR, Copy Trade, 2 years)

| Scheme | Eval Pass% | Reset% | Net Income (all) | Net Income (if pass) | Profitable% |
|:-------|:-----------|:-------|:-----------------|:---------------------|:------------|
| Conservative (A=$1K, B=$450) | 51% | 0% | $8,311 | $16,446 | 46% |
| Balanced (A=$1.5K, B=$750) | 46% | 8% | $10,098 | $22,129 | 42% |
| Aggressive (A=$2K, B=$1K) | 47% | 20% | $11,816 | $25,188 | 44% |
| **Full Send (A=$2.5K, B=$1.5K)** | **56%** | **31%** | **$16,731** | **$30,207** | **52%** |

> Full Send wins: highest eval pass rate (56%), highest net income ($16.7K), 31% reset rate (within 25-50% budget).
> Higher risk = more resets, but faster passes offset the cost.

### Sequential Pipeline: Build to 5 Funded Accounts

**Strategy:** Pass one eval at a time. Once funded, immediately start next eval.
**Budget:** 5 account slots x 2 resets each = 15 max eval attempts ($3,285 max cost)

### Rotation vs Copy Trade at 3/6/9/12 Months (Realistic 70% WR)

| Metric | 3 mo | 6 mo | 9 mo | 12 mo | 2 yr |
|:-------|-----:|-----:|-----:|------:|-----:|
| **ROTATION** | | | | | |
| Funded Active (mean) | 0.4 | 0.8 | 1.0 | 1.1 | 1.1 |
| Net Income (mean, all) | -$271 | $949 | $2,676 | $4,437 | $9,972 |
| Net Income (conditional) | -$197 | $2,100 | $5,098 | $8,216 | $18,105 |
| Profitable % | 3% | 27% | 45% | 50% | 52% |
| **COPY TRADE** | | | | | |
| Funded Active (mean) | 0.4 | 0.8 | 1.0 | 1.1 | 0.8 |
| Net Income (mean, all) | -$214 | $1,623 | $4,860 | $8,493 | $16,731 |
| Net Income (conditional) | -$48 | $3,379 | $9,045 | $15,493 | $30,207 |
| Profitable % | 4% | 31% | 47% | 51% | 52% |

### Monthly Income Estimates (if eval passes)

| Mode | Monthly | Annual (2yr avg) |
|:-----|--------:|-----------------:|
| Rotation | $754 | $18,105 |
| **Copy Trade** | **$1,259** | **$30,207** |

### Backtest-Exact Performance (80% WR, Copy Trade)

At backtest-level execution, the pipeline is dramatically better:

| Metric | 3 mo | 6 mo | 12 mo | 2 yr |
|:-------|-----:|-----:|------:|-----:|
| Funded Active | 1.6 | 3.4 | 5.0 | 5.0 |
| Net Income | $1,280 | $14,210 | $71,581 | **$188,046** |
| Profitable % | 30% | 95% | 100% | 100% |

> At backtest performance: $188K over 2 years = **$7,835/month** with Copy Trade.

### Playbook for Passing Eval Quickly

1. **Use Full Send sizing**: Type A = $2,500 risk (22 MNQ), Type B = $1,500 risk (4-9 MNQ)
2. **Focus on high-conviction entries only**: B-Day IBL Fade is the #1 eval passer
3. **Don't skip VWAP pullbacks**: Even at 4 MNQ, VWAP entries contribute $258-773/trade
4. **DD management**: If DD > $2,000, halve all risk. If DD > $3,500, stop trading.
5. **Budget 2 resets per eval slot**: Reset rate ~31% means 1 in 3 evals blow
6. **Target pass in 2-4 months**: ~11 trades needed at $818/trade avg

### After Eval: Transition to Conservative Funded

1. **Phase 1** (equity < start + $1,000): $200 risk all setups
2. **Phase 2** (DD locked): $300 risk all setups
3. **Phase 3** (buffer built $2K+): $450/$600 risk (B/A)
4. **After 6 payout cycles**: Consider transitioning to live trading account

### The Honest Assessment (v2)

- **52% of sims are profitable** at realistic 70% WR over 2 years
- **If you pass eval** (56% chance), conditional mean income is $30K/2yr with Copy Trade
- **31% reset rate** means budgeting ~$695 in eval costs per pipeline run
- **Copy Trade > Rotation** for income ($30K vs $18K conditional) but higher funded blow rate
- **The #1 variable is still live execution quality**: 80% WR -> $188K/2yr vs 70% WR -> $17K/2yr

---

*Generated by prop/sim_aggressive.py and sim_prop_firm.py*
*10,000 Monte Carlo simulations per scenario, 504 trading days (2 years)*
